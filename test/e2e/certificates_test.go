...
var _ = Describe("ACME Certificate", Ordered, func() {
	...
	Context("dns-01 challenge using explicit credentials", func() {
		It("should obtain a valid LetsEncrypt certificate", func() {
			...
			awsCredsSecret, err := loader.KubeClient.CoreV1().Secrets("cert-manager").Get(ctx, "aws-creds", metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
			...
			certDomain := "ckyal.devcluster.openshift.com" // acronym for "ACME dns-01 Route53 Explicit", short naming to pass dns name validation
