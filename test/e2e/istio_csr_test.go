...
var _ = Describe("Istio-CSR", Ordered, Label("TechPreview", "Feature:IstioCSR"), func() {
	...
	Context("configMapNamespaceSelector functionality", func() {
		...
		It("should only create istio-ca-root-cert ConfigMap in namespaces matching the selector", func() {
			By("creating istiocsr.operator.openshift.io resource with configMapNamespaceSelector")
			loader.CreateFromFile(testassets.ReadFile, filepath.Join("testdata", "istio", "istio_csr_with_namespace_selector.yaml"), ns.Name)
			defer loader.DeleteFromFile(testassets.ReadFile, filepath.Join("testdata", "istio", "istio_csr_with_namespace_selector.yaml"), ns.Name)
		...
		It("should create istio-ca-root-cert ConfigMap in all namespaces when configMapNamespaceSelector is not set", func() {
			By("creating istiocsr.operator.openshift.io resource without configMapNamespaceSelector")
			loader.CreateFromFile(testassets.ReadFile, filepath.Join("testdata", "istio", "istio_csr.yaml"), ns.Name)
			defer loader.DeleteFromFile(testassets.ReadFile, filepath.Join("testdata", "istio", "istio_csr.yaml"), ns.Name)
