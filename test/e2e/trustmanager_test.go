...
var _ = Describe("TrustManager", Ordered, Label("Feature:TrustManager"), func() {
	...
	Context("basic reconciliation", func() {
		It("should create TrustManager CR and reconcile ServiceAccount", func() {
			...
			sa, err := clientset.CoreV1().ServiceAccounts(trustManagerNamespace).Get(ctx, trustManagerServiceAccoutName, metav1.GetOptions{})
			Expect(err).ShouldNot(HaveOccurred())
			...
			err = clientset.CoreV1().ServiceAccounts(trustManagerNamespace).Delete(ctx, trustManagerServiceAccoutName, metav1.DeleteOptions{})
			Expect(err).ShouldNot(HaveOccurred())
		...
		It("should reflect spec values in status", func() {
			...
			Expect(status.SecretTargetsPolicy).Should(Equal(v1alpha1.SecretTargetsPolicyAll))
	...
	Context("error handling", func() {
		It("should set Degraded condition when trustNamespace does not exist", func() {
			nonExistentNamespace := "non-existent-namespace"

			By("creating TrustManager CR with non-existent trustNamespace")
			_, err := trustManagerClient().Create(ctx, &v1alpha1.TrustManager{
				ObjectMeta: metav1.ObjectMeta{Name: "cluster"},
				Spec: v1alpha1.TrustManagerSpec{
					TrustManagerConfig: v1alpha1.TrustManagerConfig{
						TrustNamespace: nonExistentNamespace,
					},
				},
			}, metav1.CreateOptions{})
			Expect(err).ShouldNot(HaveOccurred())

			By("waiting for TrustManager to become Degraded with correct error message")
			Eventually(func(g Gomega) {
				trustManager, err := trustManagerClient().Get(ctx, "cluster", metav1.GetOptions{})
				g.Expect(err).ShouldNot(HaveOccurred())

				// Verify Degraded condition
				degradedCondition := meta.FindStatusCondition(trustManager.Status.Conditions, v1alpha1.Degraded)
				...
				g.Expect(degradedCondition.Message).Should(And(
					...
					ContainSubstring(nonExistentNamespace),
					ContainSubstring("does not exist"),
				))

				// Verify Ready condition is False
				readyCondition := meta.FindStatusCondition(trustManager.Status.Conditions, v1alpha1.Ready)
				g.Expect(readyCondition).ShouldNot(BeNil())
				g.Expect(readyCondition.Status).Should(Equal(metav1.ConditionFalse))
				g.Expect(readyCondition.Reason).Should(Equal(v1alpha1.ReasonReady))
			}, lowTimeout, fastPollInterval).Should(Succeed())
		})
	})
})
